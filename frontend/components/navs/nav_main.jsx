...
class Nav extends React.Component {
  ...
  handleMenuMouseEnter() {
    if (!this.state.sidebarExpanded) {
      this.setState({ sidebarExpanded: true, sidebarDocked: false });
    }
  }

  handleMenuMouseLeave() {
    if (this.state.sidebarExpanded) {
      this.setState({ sidebarExpanded: false, sidebarDocked: false });
      this.setState({ sidebarUnexpanding: true, sidebarUndocking: false }),
        () =>
          setTimeout(() => {
            this.setState({ sidebarUnexpanding: false });
          }, 200);
    } else {
      null;
    }
  ...
  handleMenuClick() {
    if (this.state.sidebarDocked) {
      this.setState({ sidebarDocked: false, sidebarExpanded: false });
      this.setState(
        { sidebarUndocking: true, sidebarUnexpanding: false },
        this.rotateArrow(),
        () =>
          setTimeout(() => {
            this.setState({ sidebarUndocking: false });
          }, 200)
      );
    } else {
      this.setState({ sidebarDocked: true, sidebarExpanded: false });
    }
  }
  ...
  render() {
    ...
    let pagination = this.props.pagesPath
      ? this.props.pagesPath.map((id) => {
          let pagename =
            this.props.allNodes[id].body.length > 20
              ? this.props.allNodes[id].body.slice(0, 18).concat("...")
              : this.props.allNodes[id].body;

          return (
            <span>
              <a href={`#/nodes/${id}`} pagesPath={this.props.pathsPath}>
                {pagename}
              </a>
            </span>
          );
        ...
        <>
          <div className="navBar">
            ...
            <div className="navBarLeft">
              ...
              {!this.state.showSearchBar ? (
                <div className="searchIcon" onClick={this.handleSearchClick}>
                  <svg
                    width="20"
                    height="20"
                    viewBox=" 0 0 20 20"
